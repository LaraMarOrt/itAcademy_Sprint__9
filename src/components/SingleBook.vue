<template>
    <div class="card bg-book">
        <div class="d-flex justify-content-center">
            <img onerror="this.src='data:image/png;base64,'"
                :src="book?.volumeInfo?.imageLinks?.thumbnail" class="card-img-top">
        </div>
        <div class="card-body">
            <h3 class="card-title">{{ book?.volumeInfo?.title.toUpperCase() }}</h3>
            <p class="card-year">{{ book?.volumeInfo?.publishedDate.substr(0, 4) }}</p>
            <p v-if="book?.volumeInfo?.description" class="card-text">{{ book?.volumeInfo?.description }}
            </p>
            <p v-if="!book?.volumeInfo?.description" class="card-text">No podemos ofrecerte un resumen del
                libro, te recomendamos previsualizarlo</p>
            <div class="d-flex btn-align">
                <b-button variant="success" :href="book?.volumeInfo?.previewLink" target="_blank" v-b-tooltip.hover
                    title="Previsualizar">👁‍🗨</b-button>
                <b-button variant="warning" :href="book?.volumeInfo?.infoLink" target="_blank" v-b-tooltip.hover
                    title="Compra">📥</b-button>
                <b-button v-if="!favorite" @click="$store.dispatch('addFavourites', book)"
                    variant="warning" v-b-tooltip.hover title="Favorito">🤍</b-button>
                <b-button v-if="favorite"
                    @click="$store.dispatch('removeFavourites', book)" variant="warning" v-b-tooltip.hover
                    title="Quitar">❤</b-button>

            </div>
        </div>
    </div>
</template>

<script>

export default {
    name: 'SingleBook',
    props: [ 'book'],
    methods: {
        visualizar() {
            window.open(this.book?.volumeInfo?.imageLinks?.previewLink);
        },
    },
    computed: {
        favorite() {
            return this.$store.state.favourites.find(x=>x.id==this.book.id)!=null;
        }
    },
    }
    
</script>

<style scoped>
.card-title {
    font-size: 1.1rem;
    color: #c8fede;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
}

.btn-align {
    justify-content: space-evenly;
}

.bg-book {
    background-color: #3b4550eb;
}

.card-img,
.card-img-top,
.card-img-bottom {
    margin: 1rem;
    display: inline-block;
    position: inherit;
    width: 100px;
    height: 100px;
    overflow: hidden;
    border-radius: 50%;
}

.card-body {
    text-align: center;
    height: 200px;
}

.card-text {
    overflow: hidden;
    text-overflow: ellipsis;
    display: -webkit-box;
    -webkit-line-clamp: 2;
    line-clamp: 2;
    -webkit-box-orient: vertical;
}

.card-year {
    font-style: oblique;
    color: #c8fede;
    font-size: 0.9rem;
}

.btn {
    position: inherit;
}

.card {
    position: inherit;
}
</style>